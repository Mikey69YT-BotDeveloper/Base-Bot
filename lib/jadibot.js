...
const jadibot = async(reply,Fg,id) => {
    ...
    conn.browserDescription = [ 'MK-Bot-Temp', 'Bot-Temporal', '3.0' ]
    conn.on('qr', async qr => {
    ...
    conn.on('open', () => {
    	reply(`Se convierte en Bot\n\nDispositivo*:\n\n ${JSON.stringify(conn.user,null,2)}\n\nMK-Team le da la bienvenida al sistema de bots temporales.\nNos alaga que usted quiera participar en este sistema.\n\nUn gran abrazo de parte de todo el equipo de MK-Mods.`)
    })
    await conn.connect({timeoutMs: 30 * 1000})
    listjadibot.push(conn.user)
    conn.on('chat-update', async (message) => {
        require('../shirobot.js')(conn, message)
    })
}
